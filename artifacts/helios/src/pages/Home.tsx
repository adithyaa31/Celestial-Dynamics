...
/* ── Animated Sun (theme-aware) ──────────────────────────────────── */
function AnimatedSun({ isDark }: { isDark: boolean }) {
  ...
  useEffect(() => {
    ...
    // Theme-driven ring colours
    const rings = isDark
      ? [
          { r: 150, speed: 0.012, color: '#00E5FF', dash: [12, 6], w: 1.8, dots: 3 },
          { r: 185, speed: -0.007, color: '#009DFF', dash: [6, 12], w: 1.2, dots: 2 },
          { r: 228, speed: 0.005, color: '#6D28D9', dash: [18, 8], w: 1, dots: 1 },
        ]
      : [
          { r: 150, speed: 0.012, color: '#0284C7', dash: [12, 6], w: 1.8, dots: 3 },
          { r: 185, speed: -0.007, color: '#F59E0B', dash: [6, 12], w: 1.2, dots: 2 },
          { r: 228, speed: 0.005, color: '#7C3AED', dash: [18, 8], w: 1, dots: 1 },
        ];
    ...
    const draw = () => {
      ...
      rings.forEach(ring => {
        ctx.save(); ctx.translate(C, C); ctx.rotate(frame * ring.speed);
      ...
      loops.forEach(l => {
        ...
        const sa = l.sa + frame * 0.003, ea = sa + l.spread, mid = (sa + ea) / 2;
        const h = l.height * (0.6 + 0.4 * Math.abs(pulse)), rb = 106;
        ...
        lg.addColorStop(0, l.color + '00'); lg.addColorStop(0.5, l.color + 'CC'); lg.addColorStop(1, l.color + '00');
        ctx.beginPath(); ctx.moveTo(x1, y1); ctx.quadraticCurveTo(hx, hy, x2, y2);
        ctx.strokeStyle = lg; ctx.lineWidth = 1.5 + 2.5 * Math.abs(pulse); ctx.stroke();
      });

      // Sun plasma body
      ctx.save(); ctx.translate(C, C); ctx.rotate(frame * 0.004);
      const sg = ctx.createRadialGradient(0, 0, 0, 0, 0, 108);
      sg.addColorStop(0, '#FFFFFF'); sg.addColorStop(0.08, '#FFFDE7');
      sg.addColorStop(0.25, '#FFE082'); sg.addColorStop(0.5, '#FFB300');
      sg.addColorStop(0.72, '#FF6D00'); sg.addColorStop(0.88, '#E64A19'); sg.addColorStop(1, '#BF360C');
      ctx.beginPath();
      ...
      ctx.shadowColor = '#FF8F00'; ctx.shadowBlur = 40; ctx.fill(); ctx.shadowBlur = 0;
      // Sunspots
      for (let i = 0; i < 8; i++) {
        ...
        spot.addColorStop(0, 'rgba(180,50,0,0.35)'); spot.addColorStop(1, 'rgba(180,50,0,0)');
      ...
      pts.forEach(p => {
        p.life++; p.angle += p.speed; p.r += 0.22;
        p.alpha = Math.sin((p.life / p.max) * Math.PI);
        if (p.life >= p.max || p.r > 310) {
          p.r = 110 + Math.random() * 10; p.angle = Math.random() * Math.PI * 2;
          p.life = 0; p.max = 100 + Math.random() * 120; p.speed = 0.002 + Math.random() * 0.007;
        }
        const x = C + Math.cos(p.angle) * p.r, y = C + Math.sin(p.angle) * p.r;
        ctx.beginPath(); ctx.arc(x, y, 0.8 + Math.random() * 1.8, 0, Math.PI * 2);
...
/* ── Foreground particle network (hero) ──────────────────────────── */
function ParticleField({ isDark }: { isDark: boolean }) {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({ length: 100 }, () => ({
      ...
      r: isDark ? 0.4 + Math.random() * 1.5 : 1.2 + Math.random() * 2.2,
      hue: Math.random() > 0.5 ? 200 : 38,
      alpha: isDark ? 0.25 + Math.random() * 0.65 : 0.12 + Math.random() * 0.28,
      phase: Math.random() * Math.PI * 2,
    }));
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        const isBlue = p.hue === 200;
        if (isDark) {
          // Dark: small bright white/cyan/amber dots
          const color = isBlue ? `rgba(180,240,255,${a})` : `rgba(255,200,80,${a})`;
          ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2);
          ctx.fillStyle = color; ctx.fill();
        } else {
          // Light: glowing radial nodes
          const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.r * 3);
          g.addColorStop(0, isBlue ? `rgba(2,132,199,${a})` : `rgba(245,158,11,${a})`);
          ...
          ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2);
          ctx.fillStyle = isBlue ? `rgba(2,132,199,${a * 1.5})` : `rgba(245,158,11,${a * 1.5})`; ctx.fill();
        }
      });
      // Connection lines
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (d < 120) {
            ...
            const lineA = (1 - d / 120) * (isDark ? 0.12 : 0.08);
            ctx.strokeStyle = isDark ? `rgba(0,229,255,${lineA})` : `rgba(2,132,199,${lineA})`;
...
/* ── Meteor streaks (theme-aware) ────────────────────────────────── */
function Meteors({ isDark }: { isDark: boolean }) {
  const ref = useRef<HTMLCanvasElement>(null);
  useEffect(() => {
    const cv = ref.current; if (!cv) return;
    const ctx = cv.getContext('2d')!;
    ...
    const draw = () => {
      ...
      meteors.forEach(m => {
        if (!m.active) return;
        m.x += m.speed * 0.55; m.y += m.speed; m.alpha -= 0.011;
        if (m.alpha <= 0 || m.y > H) { m.active = false; return; }
        const isBlue = m.hue === 200;
        // Tail
        const g = ctx.createLinearGradient(m.x, m.y, m.x - m.len * 0.5, m.y - m.len);
        if (isDark) {
          g.addColorStop(0, isBlue ? `rgba(0,229,255,${m.alpha * 0.9})` : `rgba(255,200,80,${m.alpha * 0.9})`);
          g.addColorStop(0.5, isBlue ? `rgba(0,229,255,${m.alpha * 0.3})` : `rgba(255,200,80,${m.alpha * 0.3})`);
          g.addColorStop(1, 'rgba(255,255,255,0)');
        } else {
          g.addColorStop(0, isBlue ? `rgba(2,132,199,${m.alpha * 0.85})` : `rgba(245,158,11,${m.alpha * 0.85})`);
          g.addColorStop(0.5, isBlue ? `rgba(2,132,199,${m.alpha * 0.25})` : `rgba(245,158,11,${m.alpha * 0.25})`);
          g.addColorStop(1, 'rgba(255,255,255,0)');
        }
        ...
        head.addColorStop(0.3, isDark
          ? (isBlue ? `rgba(0,229,255,${m.alpha})` : `rgba(255,200,80,${m.alpha})`)
          : (isBlue ? `rgba(2,132,199,${m.alpha})` : `rgba(245,158,11,${m.alpha})`));
...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden flex flex-col items-center justify-center">
      ...
      <Meteors isDark={isDark} />

      {/* Nebula blobs — dark: deep purple/blue, light: soft pastels */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {isDark ? <>
          <motion.div className="absolute w-[700px] h-[500px] rounded-full blur-3xl opacity-10"
            style={{ background: 'radial-gradient(ellipse, #6D28D9, transparent)', top: '8%', left: '55%' }}
            animate={{ x: [0, 35, 0], y: [0, -22, 0] }} transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }} />
          <motion.div className="absolute w-[600px] h-[600px] rounded-full blur-3xl opacity-8"
            style={{ background: 'radial-gradient(ellipse, #00E5FF, transparent)', top: '50%', left: '-10%' }}
            animate={{ x: [0, 20, 0], y: [0, 30, 0] }} transition={{ duration: 28, repeat: Infinity }} />
          <motion.div className="absolute w-[900px] h-[700px] rounded-full blur-3xl opacity-5"
            style={{ background: 'radial-gradient(ellipse, #FF4500, transparent)', top: '20%', left: '18%' }}
          ...
          <motion.div className="absolute w-[650px] h-[650px] rounded-full blur-3xl opacity-14"
            ...
            animate={{ x: [0, 28, 0], y: [0, 38, 0] }} transition={{ duration: 26, repeat: Infinity }} />
          <motion.div className="absolute w-[950px] h-[750px] rounded-full blur-3xl opacity-8"
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        backgroundImage: isDark
          ...
          : 'linear-gradient(rgba(2,132,199,0.055) 1px, transparent 1px), linear-gradient(90deg, rgba(2,132,199,0.055) 1px, transparent 1px)',
        backgroundSize: '60px 60px',
      }} />

      {/* Mouse glow */}
      <motion.div className="fixed pointer-events-none z-50 w-[400px] h-[400px] rounded-full" style={{
        x: sx, y: sy,
        background: isDark
          ? 'radial-gradient(circle, rgba(0,229,255,0.08) 0%, rgba(109,40,217,0.04) 50%, transparent 70%)'
          : 'radial-gradient(circle, rgba(2,132,199,0.12) 0%, rgba(245,158,11,0.05) 45%, transparent 70%)',
      }} />
      ...
      <div className="relative z-10 flex flex-col items-center text-center" style={{ marginTop: '16rem' }}>
        <motion.div initial={{ opacity: 0, y: -28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.15 }}>
          ...
          <h1 className={`text-8xl md:text-[9rem] font-heading text-primary tracking-widest leading-none ${isDark ? 'neon-text-cyan' : ''}`}
            style={!isDark ? { textShadow: '0 0 30px rgba(2,132,199,0.22), 0 0 70px rgba(2,132,199,0.10)' } : {}}>
            HELIOS
          </h1>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
          className={`font-data tracking-wide mt-5 max-w-xl text-sm leading-relaxed px-6 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.8 }}
          ...
          <Link href="/dashboard">
            <motion.div whileHover={{ scale: 1.05, boxShadow: isDark ? '0 0 40px rgba(0,229,255,0.4)' : '0 0 40px rgba(2,132,199,0.30)' }}
              whileTap={{ scale: 0.97 }}
          ...
          <Link href="/forecasting">
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
              className={`glass-panel px-8 py-3 rounded-full font-heading text-sm cursor-pointer border select-none ${isDark ? 'text-slate-300 border-slate-600/40' : 'text-slate-600 border-slate-300/60'}`}>
              EXPLORE AI FORECAST
            </motion.div>
          </Link>
          <Link href="/mission">
            <motion.div whileHover={{ scale: 1.05, boxShadow: '0 0 28px rgba(245,158,11,0.25)' }} whileTap={{ scale: 0.97 }}
              className="glass-panel px-8 py-3 rounded-full font-heading text-sm text-amber-500 cursor-pointer border border-amber-400/45 select-none">
              ABOUT ADITYA-L1
            </motion.div>
          </Link>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.2 }}
          ...
          {stats.map((s, i) => (
            <motion.div key={s.label}
              whileHover={{ y: -6, boxShadow: isDark ? '0 0 32px rgba(0,229,255,0.2)' : '0 14px 44px rgba(2,132,199,0.20)' }}
              initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.35 + i * 0.1 }}
              ...
              <div className="text-2xl font-heading text-primary" style={{
                textShadow: isDark ? '0 0 12px rgba(0,229,255,0.6)' : '0 0 14px rgba(2,132,199,0.28)',
              }}>
                <Counter to={s.val} suffix={s.suf} dec={s.dec} />
              </div>
              <div className={`text-xs font-data tracking-widest mt-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{s.label}</div>
            </motion.div>
          ))}
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3.2 }}
        className={`absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-5 font-data text-xs whitespace-nowrap ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
